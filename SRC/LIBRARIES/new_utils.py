...
def read_net_backprop(model_name, print_out=True, default_net=None) -> 'IModelBase':
    net = read_model(model_name, print_out=print_out, default_net=default_net)
    net.train()

    return net


def read_net_inference(model_name_suffix, default_net=None) -> 'IModelBase':
    net = read_model(model_name_suffix, default_net=default_net)
    net.eval()

    return net


def produce_inference_model(model_name_suffix) -> 'IModelBase':
    try:
        module = importlib.import_module(f'SRC.NN.{model_name_suffix}')
        class_ = getattr(module, 'NN')
        model = class_()
    except FileNotFoundError as err:
        raise RuntimeError(f"NO MODEL [{model_name_suffix}] FOUND: {str(err)}")
    except Exception as ex:
        from SRC.LIBRARIES.new_data_utils import produce_signal_encoder

        model = read_net_inference(model_name_suffix)

    return model


def get_sub_net_s(_log_func=DEBUG_SPLITTED):
    with log_context(_log_func) as log_s:
        folder_path = f"{project_root_dir()}/SRC/NN"
        ...
        for file in files:
            try:
                ...
                class_ = getattr(module, 'NN')
                if class_.mro()[-5].__name__ == 'A_Base' or class_.mro()[-4].__name__ == 'A_Base':
                    net = class_(True)
                    a_base_net_name_s.append(module_name)

                    log_s.append(f'A_Base >> {net.__class__}')
                elif class_.mro()[-5].__name__ == 'S_Base' or class_.mro()[-4].__name__ == 'S_Base':
                    net = class_()
                    s_base_net_name_s.append(module_name)

                    log_s.append(f'S_Base >> {net.__class__}')
                else:
                    pass
            except Exception as ex:
                log_s.append(f"EXCEPTION: {str(ex)}")
                pass

        a_base_net_name_s = sorted(a_base_net_name_s, key=lambda s: int(s.split('_')[1]) if '_' in s else float('inf'))
        s_base_net_name_s = sorted(s_base_net_name_s, key=lambda s: int(s.split('_')[1]) if '_' in s else float('inf'))

        return {
            'A_Base': a_base_net_name_s,
            'S_Base': s_base_net_name_s,
        }
